...
exports.NoExposedMethodsError = NoExposedMethodsError;
...
// TODO add this to namespace + spec
...
function NoExposedMethodsError(message) {
...
},{"./head-governess":3}],3:[function(require,module,exports){
'use strict';
...
var HeadGoverness = exports.HeadGoverness = (function () {
  ...
  _createClass(HeadGoverness, [{
    key: 'guard',
    value: function guard(action, target) {
      var _this = this;

      if (this.isGuarded()) {
        // Is there any rule allowing the child to do that?
        var permit = _lodash._.some(this.getRules(action), function (rule) {
          return _this.isRule(rule) && rule.verify(target);
        });

        if (!permit) {
          // TODO: is there a way to get a type of target?
          throw new _errors.AccessDenied('Child is not allowed to ' + action + ' the target.');
        }
      }
    ...
    value: function getRules(type) {
      return _lodash._.filter(this.rules, function (rule) {
        return rule.type === type;
      });
    }
  }, {
    key: 'verify',
    value: function verify(action, target) {
      _lodash._.each(this.getRules(action), function (rule) {
        rule.verify(target);
      });
    ...
    value: function addRule() {
      ...
      _lodash._.each(rules, function (rule) {
        if (!_this2.isRule(rule)) {
    ...
    value: function isEmpty() {
      return _lodash._.isEmpty(this.rules);
    }
    ...
    value: function unguarded(callback, context) {
      ...
      if (_lodash._.isFunction(callback)) {
...
},{"../errors":1,"../rule":7,"lodash":10}],4:[function(require,module,exports){
"use strict";
...
var PURPOSE_REGEX = /^[a-z_\$]+[a-zA-Z0-9_\$]*$/;

/**
 * A Perimeter is used to define the places where child can play.
 */

var Perimeter = exports.Perimeter = (function () {
  ...
  _createClass(Perimeter, [{
    key: 'import',
    value: function _import(sandbox, governess) {
    ...
    value: function guard() {
      ...
      (_governess = this.governess).guard.apply(_governess, arguments);
    }
  }, {
    key: 'purpose',
    get: function get() {
    ...
    set: function set(value) {
      if (!_lodash._.isString(value) || !PURPOSE_REGEX.test(value)) {
    ...
    key: 'govern',
    get: function get() {
      return _lodash._.isObject(this._govern) ? this._govern : {};
    },
    set: function set(value) {
      this._govern = _lodash._.isObject(value) ? value : {};
    ...
    key: 'expose',
    get: function get() {
      return _lodash._.isArray(this._expose) ? this._expose : [];
    },
    set: function set(value) {
      this._expose = _lodash._.isArray(value) ? value : [];
    ...
    set: function set(value) {
      if (!_lodash._.isObject(value) || !(value instanceof _sandbox.Sandbox)) {
    ...
    set: function set(value) {
      ...
      this._governess = _lodash._.isObject(value) && value instanceof _headGoverness.HeadGoverness ? value : null;
...
},{"./errors":1,"./governesses/head-governess":3,"./sandbox":8,"lodash":10}],6:[function(require,module,exports){
'use strict';
...
var RESTRICTED_METHOD_NAMES = ['constructor', '_addPerimeter', '_name', '_sandbox'];

var Purpose = exports.Purpose = (function () {
  ...
  _createClass(Purpose, [{
    ...
    value: function _addPerimeter(perimeter) {
      ...
      if (_lodash._.isEmpty(exposedMethods)) throw new _errors.NoExposedMethodsError();

      _lodash._.each(exposedMethods, function (method) {
        if (_lodash._.include(RESTRICTED_METHOD_NAMES, name)) {
          throw new _errors.RestrictedMethodError('Method name ' + name + ' is restricted.');
        } else if (_lodash._.isFunction(_this[method])) {
          _logger.Logger.warn('Overriding already sandboxed method ' + _this.name + '.' + name + '.');
        }

        _this[name] = perimeter;
      });
    }
  }]);
...
},{"./errors":1,"./logger":4,"lodash":10}],7:[function(require,module,exports){
'use strict';
...
var Rule = exports.Rule = (function () {
  _createClass(Rule, null, [{
    ...
    value: function create(str, def) {
      var match = _lodash._.isString(str) && str.match(RULE_REGEX);

      // extract type of the rule
      // e.g. "cannot watch" => "watch"
      var type = (function () {
        return _lodash._.isArray(match) ? match[2] : undefined;
      })();

      if (_lodash._.isUndefined(type)) {
      ...
      if (_lodash._.isEmpty(items) && !_lodash._.isFunction(rule)) {
        throw new _errors.WrongRuleDefinition('Cannot parse rule "' + str + '". No items or rule given.');
      }

      return new Rule(type, items, this._createVerifyMethod(rule, isPositive, def.ruleContext), isPositive);
    }
  }, {
    key: '_createVerifyMethod',
    value: function _createVerifyMethod(rule, isPositive) {
      var ruleContext = arguments.length <= 2 || arguments[2] === undefined ? null : arguments[2];

      if (_lodash._.isFunction(rule)) {
        return function () {
          for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
            args[_key] = arguments[_key];
          }

          var res = rule.apply.apply(rule, [ruleContext].concat(args));
  ...
  function Rule(type, items, verifyMethod, isPositive) {
    ...
    if (!_lodash._.isString(this.type)) {
      throw new _errors.ArgumentError('type must be a string. ' + errorSufix);
    }

    if (!_lodash._.isArray(this.items)) {
      throw new _errors.ArgumentError('items must be an array. ' + errorSufix);
    }

    if (!_lodash._.isFunction(this.verifyMethod)) {
      throw new _errors.ArgumentError('verifyMethod must be a function. ' + errorSufix);
    }

    if (!_lodash._.isBoolean(this.isPositive)) {
      throw new _errors.ArgumentError('isPositive must be a boolean. ' + errorSufix);
    }
  }
  ...
  _createClass(Rule, [{
    ...
    value: function verify() {
      ...
      if (!_lodash._.isEmpty(this.items)) {
        isAllowed = _lodash._.some(this.items, function (item) {
...
},{"./errors":1,"lodash":10}],8:[function(require,module,exports){
'use strict';
...
var Sandbox = exports.Sandbox = (function () {
  function Sandbox(child) {
    ...
    this.child = child;
    this.governess = new _headGoverness.HeadGoverness(child);
  ...
  _createClass(Sandbox, [{
    ...
    value: function loadPerimeter() {
      ...
      _lodash._.each(perimeters, function (perimeter) {
        if (!_lodash._.isObject(perimeter) || !(perimeter.prototype instanceof _perimeter.Perimeter)) {
        ...
        perimeter.governess = _lodash._.isObject(perimeter.governess) && perimeter.governess instanceof _headGoverness.HeadGoverness ? perimeter.governess : _this.governess;

        _this.perimeters.push(perimeter);

        _this.extendPurpose(perimeter);
      });
    }
    ...
    value: function loadModule() {
      ...
      (_loadPerimeter = this.loadPerimeter).apply.apply(_loadPerimeter, [this].concat(args));
    }
  }, {
    key: 'extendPurpose',
    value: function extendPurpose(perimeter) {
      var name = perimeter.purpose;
      if (!_lodash._.isString(name)) throw new _errors.NoPurposeError();

      this.purpose[name] = this.purpose[name] || new _purpose.Purpose(name, this);
      this.purpose[name].addPerimeter(perimeter);
    }
    ...
    value: function hasPerimeter(perimeter) {
      return _lodash._.some(this.perimeters, function (p) {
    ...
    key: 'isAllowed',
    value: function isAllowed(action, target) {
    ...
    value: function isUnguarded() {
      ...
      return (_isNotGuarded = this.isNotGuarded).apply.apply(_isNotGuarded, [this].concat(args));
    }
  }, {
    key: 'subscribe',
    value: function subscribe(purposeName) {
      for (var _len4 = arguments.length, eventsAndCallback = Array(_len4 > 1 ? _len4 - 1 : 0), _key4 = 1; _key4 < _len4; _key4++) {
        eventsAndCallback[_key4 - 1] = arguments[_key4];
      }

      // FIXME: use event object instead of string
      var callback = _lodash._.last(eventsAndCallback);
      var events = eventsAndCallback.splice(-1, 1);
      var purpose = this.purpose[purposeName];

      if (!purpose) {
        throw new _errors.NoPurposeError('Sandbox does not have purpose ' + purposeName);
      }

      if (eventsAndCallback.length <= 1 || !_lodash._.isFunction(callback)) {
        throw new _errors.ArgumentError('You must provide events to subscribe to and a callback.');
      }

      _lodash._.each(events, function (e) {
        purpose._suscribe(e, callback);
      });
    }
  }, {
    key: 'unsubscribe',
    value: function unsubscribe(purposeName) {
      var purpose = this.purpose[purposeName];
      ...
      for (var _len5 = arguments.length, events = Array(_len5 > 1 ? _len5 - 1 : 0), _key5 = 1; _key5 < _len5; _key5++) {
        events[_key5 - 1] = arguments[_key5];
      }
...
},{"./errors":1,"./governesses/head-governess":3,"./perimeter":5,"./purpose":6,"lodash":10}],9:[function(require,module,exports){
'use strict';
...
var Kindergarten = {
  ...
  NoExposedMethodsError: _errors.NoExposedMethodsError,
...
},{"./kindergarten/errors":1,"./kindergarten/governesses/easy-governess":2,"./kindergarten/governesses/head-governess":3,"./kindergarten/logger":4,"./kindergarten/perimeter":5,"./kindergarten/purpose":6,"./kindergarten/rule":7,"./kindergarten/sandbox":8,"./kindergarten/version":9}]},{},[11])(11)
});